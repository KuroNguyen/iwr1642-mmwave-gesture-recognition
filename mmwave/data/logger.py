...
class Logger:
    ...
    def __set_file(self):
        ...
        if last_sample is None:
            self.log_file = os.path.join(self.get_gesture_dir(self.gesture), 'sample_1.csv')
            return
    ...
    def log(self, frame):
        ...
        if (frame is not None and
            ...
            self.empty_frames = (self.empty_frames + 'None, None, None, None, None\n')
            self.frame_num += 1
    ...
    @staticmethod
    def get_gesture_dir(gesture):
        if isinstance(gesture, str):
            gesture = GESTURE[gesture.upper()]
            return gesture.get_dir()
        return
